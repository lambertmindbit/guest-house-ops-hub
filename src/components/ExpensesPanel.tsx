    ...
    <section>
      <div className="row" style={{ justifyContent: "space-between", margin: "28px 0 14px" }}>
        <div className="row" style={{ gap: 8 }}>
          ...
          <span style={{ fontSize: 13, color: "var(--subtle)", fontWeight: 600 }}>{displayINR(total)} this period</span>
        </div>
        <button onClick={() => { setAdding(!adding); setError(null); }} className="btn btn--outline btn--sm">+ Add expense</button>
      </div>
            ...
            <div key={e.id} className="card" style={{ padding: "12px 15px", display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="row" style={{ gap: 7 }}>
                  ...
                  {e.note && <span style={{ fontSize: 12.5, color: "var(--subtle)" }}>· {e.note}</span>}
                </div>
                <div style={{ fontSize: 12, color: "var(--subtle)", marginTop: 2 }}>
                  {e.date}{e.paymentMode ? ` · ${PAYMENT_MODE_LABELS[e.paymentMode] ?? e.paymentMode}` : ""}
                </div>
              </div>
              <span className="num" style={{ fontWeight: 700, color: "var(--clay-700)" }}>−{displayINR(e.amount)}</span>
