...
| 12 (GU-4/CP-1) | ID verification flags & upload | 1 | 🟡 | schema flags `idChecked`,`idUploaded`,`idVerificationCompleted`; `src/lib/storage.ts` (Supabase adapter); `api/guests/[id]/id-document`; `IdDocumentField.tsx` | Flags + endpoint + UI done, but storage bucket is **env-gated / not activated**, and **no ID retention policy** (retention exists only for community scam/bad-guest). |
| 13 (RS-5/CM-1/CM-3) | Live guest messaging (WhatsApp/SMS/email) | 1 (Critical) | 🟡 | `src/lib/messaging.ts` (LogAdapter, `status=logged`), `message-templates.ts`, `api/messages`, `api/agent/messages`; `tests/agent-messages.test.ts`, `message-templates.test.ts` | Outbox seam works but **nothing sends** (no BSP — external limit). Also only **one** template (`bookingConfirmation`) exists — pre-arrival directions & payment-request/reminder templates + a trigger engine are **not** built. |
...
| 16 (RP-1) | Pending-payments + referral/cab/tour/reviews dashboard cards | 1 | 🟡 | `src/lib/finance.ts` `sumOutstanding()`; `tests/pending-payments.test.ts`; `src/lib/dashboard.ts` `getTodaySummary()` | Pending-payments **computed & tested** and shown on Finance, but the **Today dashboard does not include a pending-payments card** (getTodaySummary returns only occupancy/check-in/out/in-house/arrivals). Referral/cab/tour/reviews widgets not added. |
...
| 19 | Offline tolerance | 3 | 🟡 | `public/sw.js`, `public/offline.html`, `PwaRuntime.tsx`, `manifest.webmanifest` | Cache-first shell + offline fallback for navigations only. SW header states **"No offline writes"** — the recommended **queued writes** are not implemented; API data intentionally never cached. |
