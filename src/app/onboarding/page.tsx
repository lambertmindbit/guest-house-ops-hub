...
export default async function OnboardingPage() {
  const [property, roomTypes, rooms, channels, staff] = await Promise.all([
  ...
  return (
    <main className="app-main" style={{ maxWidth: 640 }}>
      <div className="entrance">
        <PageHead title="Set up your guest house" sub="A few steps to your first booking" />
        ...
        {bookable ? (
          <div className="banner banner--good" style={{ cursor: "default", marginBottom: 16 }}>
            ...
            <span style={{ flex: 1 }}>
              <b>You’re ready to take bookings.</b> Open the calendar to make your first one — or add staff below to track housekeeping.
            </span>
          </div>
        ) : (
          <div className="banner banner--warn" style={{ cursor: "default", marginBottom: 16 }}>
            ...
            <span style={{ flex: 1 }}>
              <b>{requiredRemaining} step{requiredRemaining === 1 ? "" : "s"} to go.</b> Finish the required steps below and you can start booking guests.
            </span>
          </div>
        )}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700 }}>
                  {s.label}
                  {!s.required && <span className="muted" style={{ fontWeight: 400 }}> · optional</span>}
                </div>
                <div className="muted" style={{ fontSize: "var(--fs-small)", marginTop: 2 }}>{s.hint}</div>
              </div>
        ...
        {bookable && (
          <Link href="/calendar" className="btn btn--primary btn--block" style={{ marginTop: 18 }}>
            <Icon name="calendar" size={16} /> Go to the calendar
          </Link>
        )}
      </div>
    </main>
