...
export function FaqSection({ faqs }: { faqs: Faq[] }) {
  ...
  function toBody(f: typeof EMPTY) {
    return { question: f.question.trim(), answer: f.answer.trim(), category: f.category.trim() || null };
  }
  function startEdit(f: Faq) {
    setEditId(f.id);
    setEdit({ question: f.question, answer: f.answer, category: f.category ?? "" });
  }
            ...
            <div key={f.id} className="card card--pad" style={{ padding: 12, opacity: f.active ? 1 : 0.55 }}>
              <div className="spread" style={{ gap: 10, alignItems: "flex-start" }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontWeight: 600 }}>{f.question}{f.category ? <span className="badge badge--neutral" style={{ marginLeft: 8 }}>{f.category}</span> : null}{!f.active && <span className="badge badge--neutral" style={{ marginLeft: 8 }}>Hidden</span>}</div>
